// This is a generated file. Not intended for manual editing.
package com.vesoft.jetbrains.plugin.graphdb.language.cypher.psi;

import java.util.List;
import org.jetbrains.annotations.*;
import com.intellij.psi.PsiElement;

public interface CypherReservedWord extends PsiElement {

  @Nullable
  PsiElement getKAdd();

  @Nullable
  PsiElement getKAll();

  @Nullable
  PsiElement getKAllshortestpaths();

  @Nullable
  PsiElement getKAnd();

  @Nullable
  PsiElement getKAny();

  @Nullable
  PsiElement getKAs();

  @Nullable
  PsiElement getKAsc();

  @Nullable
  PsiElement getKAscending();

  @Nullable
  PsiElement getKAssert();

  @Nullable
  PsiElement getKBegin();

  @Nullable
  PsiElement getKBy();

  @Nullable
  PsiElement getKCall();

  @Nullable
  PsiElement getKCase();

  @Nullable
  PsiElement getKCommit();

  @Nullable
  PsiElement getKConstraint();

  @Nullable
  PsiElement getKContains();

  @Nullable
  PsiElement getKCount();

  @Nullable
  PsiElement getKCreate();

  @Nullable
  PsiElement getKCsv();

  @Nullable
  PsiElement getKCypher();

  @Nullable
  PsiElement getKDelete();

  @Nullable
  PsiElement getKDesc();

  @Nullable
  PsiElement getKDescending();

  @Nullable
  PsiElement getKDetach();

  @Nullable
  PsiElement getKDistinct();

  @Nullable
  PsiElement getKDo();

  @Nullable
  PsiElement getKDrop();

  @Nullable
  PsiElement getKElse();

  @Nullable
  PsiElement getKEnd();

  @Nullable
  PsiElement getKEnds();

  @Nullable
  PsiElement getKExists();

  @Nullable
  PsiElement getKExplain();

  @Nullable
  PsiElement getKExtract();

  @Nullable
  PsiElement getKFalse();

  @Nullable
  PsiElement getKFieldterminator();

  @Nullable
  PsiElement getKFilter();

  @Nullable
  PsiElement getKFor();

  @Nullable
  PsiElement getKForeach();

  @Nullable
  PsiElement getKFrom();

  @Nullable
  PsiElement getKHeaders();

  @Nullable
  PsiElement getKIn();

  @Nullable
  PsiElement getKIndex();

  @Nullable
  PsiElement getKIs();

  @Nullable
  PsiElement getKJoin();

  @Nullable
  PsiElement getKLimit();

  @Nullable
  PsiElement getKLoad();

  @Nullable
  PsiElement getKMandatory();

  @Nullable
  PsiElement getKMatch();

  @Nullable
  PsiElement getKMerge();

  @Nullable
  PsiElement getKNode();

  @Nullable
  PsiElement getKNone();

  @Nullable
  PsiElement getKNot();

  @Nullable
  PsiElement getKNull();

  @Nullable
  PsiElement getKOf();

  @Nullable
  PsiElement getKOn();

  @Nullable
  PsiElement getKOptional();

  @Nullable
  PsiElement getKOr();

  @Nullable
  PsiElement getKOrder();

  @Nullable
  PsiElement getKPeriodic();

  @Nullable
  PsiElement getKProfile();

  @Nullable
  PsiElement getKReduce();

  @Nullable
  PsiElement getKRel();

  @Nullable
  PsiElement getKRelationship();

  @Nullable
  PsiElement getKRemove();

  @Nullable
  PsiElement getKRequire();

  @Nullable
  PsiElement getKReturn();

  @Nullable
  PsiElement getKScalar();

  @Nullable
  PsiElement getKScan();

  @Nullable
  PsiElement getKSet();

  @Nullable
  PsiElement getKShortestpath();

  @Nullable
  PsiElement getKSingle();

  @Nullable
  PsiElement getKSkip();

  @Nullable
  PsiElement getKStart();

  @Nullable
  PsiElement getKStarts();

  @Nullable
  PsiElement getKThen();

  @Nullable
  PsiElement getKTrue();

  @Nullable
  PsiElement getKUnion();

  @Nullable
  PsiElement getKUnique();

  @Nullable
  PsiElement getKUnwind();

  @Nullable
  PsiElement getKUsing();

  @Nullable
  PsiElement getKWhen();

  @Nullable
  PsiElement getKWhere();

  @Nullable
  PsiElement getKWith();

  @Nullable
  PsiElement getKXor();

  @Nullable
  PsiElement getKYield();

}
