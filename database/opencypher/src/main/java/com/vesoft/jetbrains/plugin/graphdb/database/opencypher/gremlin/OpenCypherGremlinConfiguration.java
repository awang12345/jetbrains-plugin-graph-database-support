...
public class OpenCypherGremlinConfiguration {
    ...
    public GremlinFlavor getFlavor() {
        return GremlinFlavor.valueOf(configuration.getOrDefault(FLAVOR, GremlinFlavor.GREMLIN.name()));
    }

    public Serializers getSerializer() {
        return valueOf(configuration.getOrDefault(SERIALIZER, GRAPHSON_V2D0.name()));
    }
